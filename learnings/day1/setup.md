...
**Installation :**  
Make sure, environment variable/path variable is set while installing, so that python could be accessed from anywhere in the system. (usually comes as "Add to Path")
...
a. Open cmd, paste this and enter:
```bash
...
If HTTPS + PAT authentication mechanism is chosen :
```bash
git clone https://github.com/mukeshcode/SmartReply.git
```

If SSH authentication mechanism is chosen :
```bash
git clone git@github.com:mukeshcode/SmartReply.git
```
